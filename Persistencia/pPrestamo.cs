...
namespace Persistencia
{
    public class pPrestamo : CapaDatos
    {
                                    ...
                                    int anulado)
        {
            ...
            sql = "INSERT INTO prestamo (socio_id, socio_nro, fecha, hora, monteopedido, tasa, cantidadcuotas, importecuota, numeroPrestamoAnt, montopedidoAnt, amortizacionVencer, interesesVencer, cuotasPactadas, cuotasPagadas, cuotaAnt, tasaanterior, anulado) VALUES (" + id_socio + ",'" + socio_nro.Replace("-", "").Replace(".", "").Replace(",", "") + "','" + fecha.ToString("yyyy/MM/dd") + "','" + hora.ToString("yyyy/MM/dd hh:mm:ss") + "'," + monteopedido.ToString("##0.00").Replace(",", ".") + "," + tasa.ToString("##0.00").Replace(",", ".") + ", " + cantidadcuotas.ToString().Replace(",", ".") + " ," + importecuota.ToString("##0.00").Replace(",", ".") + "," + numeroPrestamoAnt.ToString().Replace(",", ".") + "," + montopedidoAnt.ToString("##0.00").Replace(",", ".") + ", " + amortizacionVencer.ToString("##0.00").Replace(",", ".") + "," + interesesVencer.ToString("##0.00").Replace(",", ".") + "," + cuotasPactadas.ToString().Replace(",", ".") + "," + cuotasPagadas.ToString().Replace(",", ".") + "," + cuotaAnt.ToString().Replace(",", ".") + "," + tasaanterior.ToString("##0.00").Replace(",", ".") + "," + anulado + ");" + "Select last_insert_id()";
